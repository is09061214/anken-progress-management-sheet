...
def _find_col_before(header: list, before: Optional[int], *keywords: str) -> Optional[int]:
    """`before` 列より左で、キーワードを含む「最も右」の列インデックスを返す。

    「ダッシュボード」シートは要対応表（左）と情報不足表（右）が同じ行に
    左右並びで置かれている。情報不足表の列を取るとき、左表の同名見出し
    （クライアント/タイトル など）を誤って拾わないよう、基準列（不足項目）の
    すぐ左側＝情報不足表側の列だけを対象にする。
    """
    best = None
    for i, cell in enumerate(header):
...
def parse_dashboard(grid: list[list], today: date) -> DashboardSnapshot:
    ...
    if g_idx is not None:
        h = grid[g_idx]
        # 「不足項目」列を基準に、その左隣＝情報不足表側の列だけを拾う
        # （左の要対応表の同名見出しを誤って拾わないようにするため）。
        c_missing = _find_col(h, "不足")
        c_client = _find_col_before(h, c_missing, "クラアント", "クライアント", "ライアント")
        c_title = _find_col_before(h, c_missing, "タイトル")
        c_pub = _find_col_before(h, c_missing, "投稿予定", "公開予定", "投稿")
...
def _sample_grid() -> list[list[str]]:
    ...
    パーサーの動作確認ができる。
    """
    # 実シートと同じく、要対応表（左: 列0-7）と情報不足表（右: 列9-12）を
    # 同じ行に左右並びで配置する。間（列8）はスペーサー。
    criteria = (
        "判定基準：🔴要対応＝公開7日以内　／　"
        "🟡もうすぐ＝締切超過 or 破綻　／　🔵順調＝それ以外・完了　／　"
